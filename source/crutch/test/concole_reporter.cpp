...
void ConsoleReporter::TestStarted(const ITest& test) noexcept {
  std::printf("--------------------------------------------------------------------------------\n");
  std::printf("Run test \033[34m%s\033[30m from suite \033[34m%s\033[30m\n", test.Name().Data(), suite_name_.Data());
}

void ConsoleReporter::TestPassed(const ITest& test) noexcept {
  std::printf("Test \033[34m%s\033[30m \033[32mpassed\033[30m\n", test.Name().Data());
  std::printf("\033[36mNote:\033[30m elapsed time\n\n");
}

void ConsoleReporter::AssertionFailureOccurred(const ITest& test, const AssertionFailure& assertion_failure) noexcept {
  std::printf("Test \033[34m%s\033[30m \033[32mfailed\033[30m by assertion\n", test.Name().Data());
  std::printf("\033[31mAssertion failure:\033[30m %s in %s:%zu\n\n",
...
void ConsoleReporter::UnhandledExceptionThrown(const ITest& test, const std::exception& exception) noexcept {
  std::printf("Test \033[34m%s\033[30m \033[32mfailed\033[30m by exception\n", test.Name().Data());
  std::printf("\033[31mWhat:\033[30m %s\n\n", exception.what());
}

void ConsoleReporter::UnhandledUnknownExceptionThrown(const ITest& test) noexcept {
  std::printf("Test \033[34m%s\033[30m \033[32mfailed\033[30m by unknown exception\n", test.Name().Data());
  std::printf("\033[31mWhat:\033[30m ?\n\n");
}
