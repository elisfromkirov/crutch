...
class ConsoleReporter : public ITestReporter {
 public:
  ...
  void AssertionFailureOccurred(const ITest& test,
                                const AssertionFailure& assertion_failure) noexcept override;

  void UnhandledExceptionThrown(const ITest& test,
                                const std::exception& exception) noexcept override;
