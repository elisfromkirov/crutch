...
TEST_SUITE(Vector) {
  ...
  TEST(EmplaceBack) {
    crutch::Vector<crutch::Complex> vector{};
    ...
    for (crutch::SizeType iteration = 0; iteration < kIterations; ++iteration) {
      int value{static_cast<int>(gen())};
      crutch::CopyOnly copy_only{value};
      crutch::MoveOnly move_only{value};
      vector.EmplaceBack(copy_only, std::move(move_only));
      ASSERT_EQ(vector.Back().value, value * 2);
    }
  }
